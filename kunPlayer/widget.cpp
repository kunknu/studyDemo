...
Widget::Widget(QWidget *parent): QWidget(parent), ui(new Ui::Widget)
{
    ...
    qRegisterMetaType<kunplay_thread ::PlayState>();
...
void Widget::setTitle(QString str)
{
    ui->label_titleName->setText(str);
    this->setWindowTitle(str);
}

void Widget::slotTotalTimeChanged(qint64 uSec)
{
...
void Widget::slotStateChanged(kunplay_thread::PlayState state)
{

    if (state == kunplay_thread::Stop)
    {
        ui->widget_video->hide();
        ui->widget_showopen->show();
        ui->pushButton_pause->hide();
    }
    else if (state == kunplay_thread::playing)
    {
    ...
    else if (state == kunplay_thread::Pause)
    {
        ui->pushButton_pause->hide();
...
void Widget::checkCursorDirect(const QPoint &cursorGlobalPoint)
{
...
void Widget::showBorderRadius(bool isShow)
{
    QString str;
...
void Widget::doChangeFullScreen()
{
    if (this->isFullScreen())
    {
        this->doShowNormal();
